<%-- 
    Document   : vender_base
    Created on : 2019/7/9, 下午 02:46:45
    Author     : user
--%>
...
<% /*-------------- pageInit.jsp 宣告之公用變數如下 -------------------*/
 ...
 /*--------------------------------------------------------------------*/
    //填空區一 ,請注意:設定程式選單的主題
    pgTitle = "供應商總商品維護";      // 作業名稱
    funcTitle = "";                           // 功能名稱
    pgURL = "vender_base.jsp";        // 程式名稱   
    menuSno = "3";                     // 選單的群組之明細定義-請參考代碼自行修改各程式所屬的功能群組  
    maxPagelist=5;
    /*填空區一二 ,功能連結宣告----------------------------*/
    String addFuncDesc = "新增供應商商品";
    String addFunc = "vender_baseADDui.jsp";
    String editFuncDesc = "&nbsp;修改";
    String editFunc = "vender_baseEDITui.jsp?editMode=E&prodID=";
    String deletFuncDesc = "&nbsp;刪除";
    String deletFunc = "./vender_baseEDITui.jsp?editMode=D&prodID=";

    /*---------------------------------------------------------------------*/
    //填空區三 ,請注意:開始設定本程式程式的查詢邏輯
    String pdVenderID = request.getParameter("pdVenderID");
    String mainSQL = "SELECT * FROM prod_base inner join vender on vender.pdVenderID = prod_base.pdVendorID"; // 預設查詢主體
    String countSQL = "SELECT count(*) AS count FROM prod_base inner join vender on vender.pdVenderID = prod_base.pdVendorID";          // 預設查詢主體的資料庫數量SQL  
    String defFilterSQLcomm = " where prod_base.pdVendorID='"+sysuserID+"' order by prodID desc";     // 預設查詢-過濾主體的SQL   
    String verFilterSQL = " where prod_base.pdVendorID='"+sysuserID+"' and name1 LIKE '%" + DBcomic.escapeString(searchkey) + "%' order by prodID desc"; // 變化組合的查詢條件SQL
    // 組合過濾條件 :不需異動
                        ...
                        <div class="content-panel">
                            ...
                            <form class="form-inline">
                                <div class="form-group">
                                    ...
                                    <input type="text" value="<%=searchkey%>" placeholder="請填入商品關鍵字" name="searchkey" id="search" class="form-control" size="30">
                                </div> 
                                ...
                                <table class="table table-bordered table-striped table-condensed cf">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td data-title="商品"> <%=vender.get("name1")%></td>
                                            <td data-title="類別"><%=  DBcomic.getCodeDescStr(vender.get("prodtype").toString() )  %></td>
                                            <td data-title="定價"><%=vender.get("price")%></td>
                                            <td data-title="庫存量"><%=vender.get("inventory")%></td>
                                            <td data-title="異動"> 
                                                <div>
                                                    <a class="btn btn-primary btn-sm" role="button" href="<%=editFunc%><%=vender.get("prodID")%>"><i class="fas fa-pen"></i><%=editFuncDesc%></a>
                                                    <!--<a class="btn btn-danger btn-sm" role="button"  href="<%=deletFunc%><%=vender.get("prodID")%>"><i class="fas fa-trash-alt"></i><%=deletFuncDesc%></a>-->
